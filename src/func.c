...
int read_file(const char *input_file, unsigned char **out, long *size,
	      wav_header_t *header)
{
	...
	while (1) {
		...
		if (fread(chunk_header, 1, 4, wav_file) != 4) {
			fprintf(stderr,
				"Error: Reached end of file without finding 'data' chunk.\n");
			fclose(wav_file);
			return 1;
		}
...
int convert_to_samples(unsigned char *data_buffer, double **sample_data,
		       long *size, int bits_per_sample)
{
	int i = 0;
	...
	*sample_data = (double *)calloc(*size, sizeof(double));
	for (i = 0; i < *size; i++) {
		(*sample_data)[i] = get_sample(
			&data_buffer[i * bytes_per_sample], bytes_per_sample);
	}
...
int hamming(double *sub_window, int sub_size)
{
	...
	for (i = 0; i < sub_size; i++) {
		hamming_value =
			0.54 - 0.46 * cos(2 * M_PI * i / (sub_size - 1));
...
int create_periodogram(double *sub_window, int sub_size, int per_size,
...
int fft_to_mel(double *fft_values, double *mel_points, double delta_f)
{
	...
	for (i = 0; i < LINEKEY_SIZE + 1; i++) {
		mel_points[i] =
			700 * pow(10, ((double)mel_points[i] / 1127.01048) - 1);
	}
...
int pass_periodogram_through_mel_filter(double *periodogram, double *result,
...
int threshold_function(double *psd, double *out_func)
{
	...
	for (i = 0; i < LINEKEY_SIZE; i++) {
		out_func[i] =
			(a * exp(b * i)) + THRESHOLD_BIAS; // eq. (6) in paper
	}

	return 0;
}

int generate_linekey(double *window_data, int sigma_size, int sub_size,
		     int sample_rate, linekey_t *out_linekey)
{
	//variable
	...
	double **mel_filter_results =
		NULL; // total of LINEKEY_SIZE array each the result of a single mel filter
	double delta_f = 0;
	int step_size = 0;
	int per_size = 0;
	int i = 0;
	...
	for (i = 0; i < NUM_OF_PERIODOGRAMS; i++) {
		...
		create_periodogram(sub_window, sub_size, per_size, in, out, p,
				   periodograms[i]); // eq. (2) in paper
	}
	...
	mel_filter_results =
		(double **)calloc(NUM_OF_PERIODOGRAMS, sizeof(double *));
	fft_to_mel(fft_values, mel_points, delta_f);
	// calculate mel results for each periodogram
	for (i = 0; i < NUM_OF_PERIODOGRAMS; i++) {
		mel_filter_results[i] =
			(double *)calloc(LINEKEY_SIZE, sizeof(double));
		for (j = 1; j < LINEKEY_SIZE + 1; j++) {
			pass_periodogram_through_mel_filter(
				periodograms[i], &mel_filter_results[i][j - 1],
				mel_points[j - 1],
				mel_points[j]); // eq. (3) in paper
		}
	}
	...
	for (i = 0; i < LINEKEY_SIZE; i++) {
		out_linekey->binary_values =
			(int *)calloc(LINEKEY_SIZE, sizeof(int));
...
int analyze_data(double *audio_data, long data_size, wav_header_t header,
		 int song_idx, linekey_t *unique_linekeys, int *unique_len)
{
	...
	if (!unique_linekeys && *unique_len) {
		fprintf(stderr,
			"Error: unique linekey length != 0 but pointer is null.\n");
		return_val = 1;
		goto out;
	...
	ms_size = (header.sample_rate * header.num_channels) /
		  1000; // represent the number of samples in a single ms
	...
	for (i = 0; i < data_size; i += step_size) {
		...
		if (cur_linekey >= *unique_len) {
			unique_linekeys = (linekey_t *)realloc(
				unique_linekeys,
				(*unique_len + 50) * sizeof(linekey_t));
			*unique_len += 50;
		}
		generate_linekey(cur_window, sigma_size, sub_size,
				 header.sample_rate,
				 &unique_linekeys[cur_linekey]);
		...
		if (!unique_linekeys[cur_linekey].position_arr &&
		    unique_linekeys[cur_linekey].pos_arr_len) {
			fprintf(stderr,
				"Error: position array length != 0 but pointer is null.\n");
			return_val = 1;
			goto out;
		}
		// if array is empty (first song input) then create a new array
		else if (!unique_linekeys[cur_linekey].position_arr) {
			unique_linekeys[cur_linekey].position_arr =
				(int **)calloc(20, sizeof(int *));
			unique_linekeys[cur_linekey].pos_arr_len = 20;
		}
		// expand array if full
		else if (unique_linekeys[cur_linekey].new_pos >=
			 unique_linekeys[cur_linekey].pos_arr_len) {
			unique_linekeys[cur_linekey]
				.position_arr = (int **)realloc(
				unique_linekeys[cur_linekey].position_arr,
				(unique_linekeys[cur_linekey].pos_arr_len +
				 20) * sizeof(int *));
			unique_linekeys[cur_linekey].pos_arr_len += 20;
		...
		unique_linekeys[cur_linekey]
			.position_arr[unique_linekeys[cur_linekey].new_pos] =
			(int *)calloc(2, sizeof(int));
		unique_linekeys[cur_linekey]
			.position_arr[unique_linekeys[cur_linekey].new_pos][0] =
			song_idx;
		unique_linekeys[cur_linekey]
			.position_arr[unique_linekeys[cur_linekey].new_pos][1] =
			song_idx;
